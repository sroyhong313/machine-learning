...
class Network(object):
    """ The list sizes contains the number of neurons in the respective layers.
        So, for example, if we want to create a Network object with 2 neurons in the first layer,
        3 neurons in the second layer, and 1 neuron in the final layer (Network([2,3,1])).
        ...
        with mean 0 and standard deviation 1. This random initialization gives our
        stochastic gradient descent algorithm a place to start from. Note that the
        Network initialization code assumes that the first layer of neurons is an input layer,
        and omits to set any biases for those neurons, since biases are only ever used in
    ...
    def feedforward(self, a):
        '''Return output of network if "a" is input. Assumes a is (n,1) Numpy ndarray '''
